...
namespace AppEx.Services.CSV
{
    ...
    public class CsvService : ICsvService
    {
        ...
        protected CsvWaterConnectOptions Options { get; }

        public CsvService(IConfiguration configuration)
        {
            Options = new CsvWaterConnectOptions();
        ...
        public List<ExpandoObject> ConvertToRecords(StreamReader reader)
        {
            ...
            {
                var records = csv.GetRecords<CsvWaterConnectItem>().ToList();
        ...
        public string SaveAs(List<ExpandoObject> records, string filename, bool force)
        {
            ...
            using (var writer = new StreamWriter(path))
            {
                ...
                {
                    var transformedRecords = new List<CsvWaterConnectTranformItem>();
                    if (records?.Count > 0)
                    {
                        records.ForEach(item =>
                        {
                            var transformItem = item.ConvertTo<CsvWaterConnectTranformItem>();
        ...
        public ExpandoObject TransformRecord(CsvWaterConnectItem csvItem)
        {
